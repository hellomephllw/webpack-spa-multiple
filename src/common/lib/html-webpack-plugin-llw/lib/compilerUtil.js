    ...
    function emitFiles(err) {
        ...
        require("async").forEach(Object.keys(compilation.assets), function(file, callback) {
            ...
            function writeOut(err) {
                ...
                var fragments = targetPath.split('\\');
                if (/.html$/.test(fragments[fragments.length - 1])) {
                    console.log(fragments[fragments.length - 1]);
                    // var contentUtf8 = iconv.decode(content, 'GBK');
                    // console.log(contentUtf8);
                    var contentGbkBuff = iconv.encode(content.toString(), 'GBK');
                    // console.log(contentGbkBuff.toString());
                    content = contentGbkBuff;
                }
